        ...
        try {
            if (isset($id) && intval($id) !== 0) {
                ...
                if (!$this->canPerformAction($place->userId)) {
                    Log::info(\AurageoConstants::CANNOT_PERFORM_ACTION_LOG . "DELETE Multimedia Avatar, USER_ID: $place->userId, CURRENT_USER_ID: " . isset($this->currentUser->id) ? $this->currentUser->id : 0 . ", PLACE_ID: $id");
    ...
    public function uploadCardImage($id)
    {
        try {
            ...
            if (isset($id) && intval($id) !== 0) {
                ...
                if (!$this->canPerformAction($place->userId)) {
                    Log::info(\AurageoConstants::CANNOT_PERFORM_ACTION_LOG . "DELETE Multimedia Card, USER_ID: $place->userId, CURRENT_USER_ID: " . isset($this->currentUser->id) ? $this->currentUser->id : 0 . ", CARD_ID: $id");
                    return response(\AurageoConstants::CANNOT_PERFORM_ACTION, Response::HTTP_FORBIDDEN);
                }
